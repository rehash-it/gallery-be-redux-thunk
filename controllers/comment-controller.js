...
exports.galleryComments = async (req, res) => {
  ...
  const comments = await Comment.find({ gallery_id }).populate(
    "user_id",
    "_id username email image account_type"
  );
  const arrangedComments = arrangeComments(comments);
  const page = parseInt(p);
  const limit = parseInt(l);
...
exports.userComments = async (req, res) => {
  ...
  const apiFeatures = new APIFeatures(
    Comment.find({ user_id }).populate("gallery_id"),
    req.query
  )
    .filter()
    .sort()
...
exports.updateComment = async (req, res) => {
  ...
  if (!comment)
    return sendError("The Comment Category with the given ID was not found.");
